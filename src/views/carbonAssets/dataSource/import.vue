<template>
  <div class="divBox">
    <el-card class="box-card">
      ...
      <el-form ref="formValidate" v-loading="fullscreenLoading" class="formValidate mt20" :rules="ruleValidate" :model="formValidate"
        ...
        <el-row v-show="currentTab === 0" :gutter="24">
          ...
          <el-col v-bind="grid2">
            <el-form-item label="商品分类：" prop="cateIds">
              <el-cascader v-model="formValidate.cateIds" :options="merCateList" :props="props2" clearable class="selWidth"
          ...
          <el-col :span="24">
            <el-form-item label="优惠券：" class="proCoupon">
              <div class="acea-row">
                <el-tag v-for="(tag, index) in formValidate.coupons" :key="index" class="mr10" closable
          ...
          <el-col :span="10">
            <el-form-item label="商品绿度：" prop="greenness">
              <el-select v-model="formValidate.greenness" placeholder="请选择商品绿度" class="selWidth">
                <!--  <el-option label="五级" :value="五级"></el-option>
                 <el-option label="四级" :value="四级"></el-option>
                 <el-option label="三级" :value="三级"></el-option>
                 <el-option label="二级" :value="二级"></el-option>
                 <el-option label="一级" :value="一级"></el-option> -->
                <el-option v-for="item in optionData" :label="item.name" :key="item.value" :value="item.value"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
        </el-row>
          ...
          <el-col :span="24">
            <el-form-item label="活动优先级：">
              <div class="color-list acea-row row-middle">
                <div class="color-item" :class="activity[item]" v-for="item in formValidate.activity" :key="item"
          ...
          <el-col v-if="formValidate.specType" :span="24" class="noForm">
            ...
            <el-form-item>
              <div v-for="(item, index) in formValidate.attr" :key="index">
                <div class="acea-row row-middle"><span class="mr5">{{ item.attrName }}</span><i class="el-icon-circle-close"
                    @click="handleRemoveAttr(index)" /></div>
                <div class="rulesBox">
                  ...
                  <el-input v-if="item.inputVisible" ref="saveTagInput" v-model="item.attrValue.attrsVal" class="input-new-tag"
                    size="small" @keyup.enter.native="createAttr(item.attrValue.attrsVal,index)" @blur="createAttr(item.attrValue.attrsVal,index)" />
            ...
            <el-form-item v-if="formValidate.attr.length>0 && formValidate.specType" label="商品属性：" class="labeltop">
              <el-table :data="ManyAttrValue" border class="tabNumWidth" size="mini">
                <template v-if="manyTabDate">
                  <el-table-column v-for="(item,iii) in manyTabDate" :key="iii" align="center" :label="manyTabTit[iii].title"
                    min-width="80">
                    <template slot-scope="scope">
  ...
  export default {
    ...
    methods: {
      ...
      onChangeGroup() {
        ...
        this.checkboxGroup.includes('isBenefit') ? this.formValidate.isBenefit = true : this.formValidate.isBenefit =
          false
            ...
            arr.forEach((v, i) => {
              ...
              data.forEach(function(vv) {
                arr[i + 1] && arr[i + 1]['attrValue'] && arr[i + 1]['attrValue'].forEach(g => {
                  const rep2 = (i !== 0 ? '' : arr[i]['attrName'] + '_') + vv + '$&' + arr[i + 1][
                    'attrName'
                  ] + '_' + g
                  tmp.push(rep2)
                  if (i === (arr.length - 2)) {
        ...
        this.$refs[name].validate((valid) => {
          if (valid) {
            ...
            if (!this.formValidate.storeName || !this.formValidate.cateId || !this.formValidate.keyword ||
              !this.formValidate.unitName || !this.formValidate.storeInfo || !this.formValidate.image || !this.formValidate
              .sliderImages) {
              this.$message.warning("请填写完整商品信息！");
            }
          }
